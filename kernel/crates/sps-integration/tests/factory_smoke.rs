...
fn factory_smoke_test_full_chain() {
    ...
    // Verify factory run has 8 completed stages (Phase 11A: all stages now materialized).
    ...
    if stages == 8 {
        println!("  PASS — FactoryRun.completed_stages == 8 (all stages materialized)");
    } else {
        println!("  FAIL — completed_stages == {} (expected 8)", stages);
